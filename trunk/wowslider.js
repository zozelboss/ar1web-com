...
$(".widget-content").each(function(){var e=$(this).text();if(e.substr(0,10).match("fbbox")){e=e.replace("fbbox/","");$(this).html('<iframe allowtransparency="true" frameborder="0" scrolling="no" src="//www.facebook.com/plugins/likebox.php?href='+e+'&width=340px&height=258&colorscheme=light&show_faces=true&header=false&stream=false&show_border=false&appId=492409184153294" style="border:none; overflow:hidden; width:100%; height:258px;"></iframe>');}});$(document).ready(function() {$('#sidebar-wrapper h2:contains("فيس بوك")').parents(".widget").addClass('facebook-box');});
    (function($){
        $('.data-snippet').each(function() {
          var postdesc = $(this);
          $.get(postdesc.attr('data-post'), function (data) {
            postdesc.html('<span>'+$(data).find('.post-body').text().substr(0, 300)+' [...]</span>');   
          },"html");
       });
    })(jQuery);
    ...
    $('.index .post-outer').each(function() {
        var title = $(this).find('.post-home .post-title').text();
        var thumb = $(this).find('.post-thumb a').attr('data-thumb');
        var snippet = $(this).find('.data-snippet');
        var url = snippet.attr('data-post');
        var share = $(this).find('.post-share').length;
        if (share === 0) {
          snippet.after('<div class="post-share"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=' + url + '"><i class="fa fa-facebook"></i></a><a target="_blank" href="https://twitter.com/intent/tweet?source=webclient&text=' + title + ' ' + url + '"><i class="fa fa-twitter"></i></a><a target="_blank" href="https://plus.google.com/share?url=' + url + '"><i class="fa fa-google-plus"></i></a><a target="_blank" href="http://www.pinterest.com/pin/create/button/?source_url=' + url + '&media=' + thumb + '&description=' + title + '"><i class="fa fa-pinterest"></i></a><a target="_blank" href="http://www.linkedin.com/shareArticle?mini=true&url=' + url + '"><i class="fa fa-linkedin"></i></a><div>');
        }
    });
  if ( $('.item .first-img').length != 0) {
    $('.post-body img:first-child').remove();
  }
...
$( ".FollowByEmail .follow-by-email-inner form table" ).html( "<tr><td><input class='follow-by-email-address' name='email' placeholder='البريد الإلكتروني..' type='text'><div class='email-icon'><i class='fa fa-envelope'/></div></td><td width='64px'><input class='follow-by-email-submit' type='submit' value='إشتراك'></td></tr>" );
});
$(document).on('click','#search-button button.open',function(){
  $( "#search-bar" ).css("display","block");
  $( "#search-bar" ).animate({right:'0px'},"500");
  $( this ).css("display","none");
  $( "#search-button button.remove" ).css("display","block");
      });
$(document).on('click','#search-button button.remove',function(){
  $( "#search-bar" ).css("display","none");
  $( "#search-bar" ).css("right","100%");
  $( this ).css("display","none");
  $( "#search-button button.open" ).css("display","block");
      });
// Infinite Scrolling
(function($) {
var loadingGif = 'http://1.bp.blogspot.com/-mS3MxrWoZMA/U7cGlLXgvVI/AAAAAAAAEQE/OYVpjUSqEHI/s1600/ajax-loader+(2).gif';
var olderPostsLink = '';
var loadMoreDiv = null;
var postContainerSelector = '.blog-posts';
var loading = false;
var win = $(window);
var doc = $(document);
...
function loadMore() {
  ...
  loading = true;
  if (!olderPostsLink) {
    loadMoreDiv.html('<span>لايوجد المزيد</span>');
    return;
  }
...
function init() {
  ...
  var link = $('<a href="javascript:;"><i class="fa fa-spin fa-circle-o-notch"></i></a>');
  link.click(loadMore);
  var img = $('<img src="' + loadingGif + '" style="display: none;">');
  win.scroll(handleScroll);
  loadMoreDiv = $('<div class="loadmore"></div>');
...
$(".slider-layout .HTML .widget-content").each(function () {
  var tag = $(this).text();
    $.ajax({
      url: "/feeds/posts/default/-/" + tag + "?alt=json-in-script&max-results=9",
      type: 'get',
      dataType: "jsonp",
        ...
        for (var i = 0; i < data.feed.entry.length; i++) {
          ...
          if (content.indexOf("http://www.youtube.com/embed/") > -1 || content.indexOf("https://www.youtube.com/embed/") > -1 ) {
            ...
            var src = $content.find('img:first').attr('src');
              ...
              var thumb = '<a class="recent-thumb" href="' + posturl + '" style="background:url(http://3.bp.blogspot.com/-qnLm52EsvBE/VDkrZ46TWXI/AAAAAAAAAsM/tiJ36WiboU4/s1600/90.jpg) no-repeat center center;background-size: cover"/>';
          }
